...
class ChatGPTTelegramBot:
    ...
    async def price_list(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        Show price list
        """
        if not await is_allowed(self.config, update, context):
            logging.warning(
                f"User {update.message.from_user.name} (id: {update.message.from_user.id}) "
                f"is not allowed to this conversation"
            )
            await self.send_disallowed_message(update, context)
            return

        logging.info(
            f"Showing price list for user {update.message.from_user.name} "
            f"(id: {update.message.from_user.id})..."
        )

        # Create an inline keyboard with a single button
        keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton("Оплатить подписку", url="https://t.me/iliashe4ka")]]
        )
        chat_id = update.effective_chat.id
        payment_message = await context.bot.send_message(
            chat_id=chat_id,
            text="""
        Привет! Рад сообщить тебе о стоимости нашей подписки. У нас два варианта:

        1. Подписка стоит 250 рублей в месяц. В рамках этой подписки ты получаешь безлимитное количество запросов, 50 изображений и 50 документов.

        ⭐️ Безлимит на использование GPT-3.5 16k токенов;
        ⭐️ 50 генераций изображений Dalle3;
        ⭐️ 50 загрузок документов (txt, docx, pdf) для Вопрос-Ответов по материалам;

        2. Если тебе нужен полный безлимит на все функции, то стоимость подписки составляет 500 рублей в месяц.

        ✅ Безлимит на использование GPT-3.5 16k токенов;
        ✅ Безлимит на генерацию изображений Dalle3;
        ✅ Безлимит на загрузку документов (txt, docx, pdf) для Вопрос-Ответов по материалам.

        Если у тебя возникнут дополнительные вопросы или нужна помощь, не стесняйся обращаться!
                    """,
    ...
    def run(self):
        ...
        application.add_handler(CommandHandler("price_list", self.price_list))
